...
export default async function handler(req, res) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (p) {
          ...
          const baseDesc  = p.desc || 'TECHNO INFODZ';
          desc = priceLine ? `${priceLine} — ${baseDesc}` : baseDesc;
  ...
  // Facebook تاريخياً أقل موثوقية مع صور WebP في og:image من jpg/png،
  // لذا نحدّد النوع الصحيح صراحة لمساعدة الزاحف على تفسير الصورة بدقة
  const extMatch = img.match(/\.(jpe?g|png|webp|gif)(\?|#|$)/i);
