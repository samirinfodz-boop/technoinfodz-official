 ...
 * ────────────────────────────────
 * يقرأ data.json و content.json من GitHub ويحدّث الموقع الرئيسي تلقائياً
 * بالإضافة لتتبع الزيارات المباشرة ونوع الجهاز وإرسالها للوحة التحكم
 */
...
(async function(){
  ...
  // ── 1. TRACK VISITOR (تتبع الزوار والعدادات تلقائياً) ──
  async function trackVisitor() {
    try {
      const SITE_NAMESPACE = "technoinfodz_official_site";
      const now = new Date();
      ...
      const weekNum = Math.ceil((days + startDate.getDay() + 1) / 7);
      const weekKey = `${yearKey}_w${weekNum}`;

      // تحديد نوع الجهاز (هاتف أم حاسوب)
      const isMobile = /Mobi|Android/i.test(navigator.userAgent);
      const devKey = isMobile ? 'dev_mobile' : 'dev_desktop';

      // إرسال إشارة زيارة لكل العدادات
      const targets = [`day_${todayKey}`, `week_${weekKey}`, `month_${monthKey}`, `year_${yearKey}`, devKey];
      targets.forEach(key => {
        fetch(`https://api.counterapi.dev/v1/${SITE_NAMESPACE}/${key}/up`).catch(() => {});
      });
    } catch(e) {}
  }
  ...
  // ── 3. LOAD CONTENT (Hero / About / Contact) ──
